...
- 点击“检测网维无盘”后，软件才会读取当前电脑的 UEFI 固件启动项，识别 `PXE / IPv4 / IPv6 / Network` 等网络启动入口；不会写死某台电脑的 GUID。
- 检测窗口会显示启动项名称、网络类型、固件 GUID、设备参数和 EFI 路径。确认参数后，才会启用醒目的“启动网维无盘”按钮。
- 如果固件不暴露网络启动项，会显示明确的检测结果，原有本地系统切换功能不受影响。
...
需要进入网维无盘时，点击“检测网维无盘”。软件会先读取本机 UEFI 网络启动参数，只有检测到可用的网络启动项后才允许点击“启动网维无盘”。该操作会再次确认，确认后仅写入下一次启动的 UEFI 启动项并立即重启，不会修改 BIOS 的永久启动顺序。
...
- “启动网维无盘”必须经过手动检测和参数核对，并在确认后才会设置下一次 UEFI 启动项。
...
- 主界面采用 macOS HIG 启发的单页主控台：全宽顶部融合 Banner、中部 6:4 系统列表与 Inspector、底部渐进式网维卡片，不再保留侧边导航栏。
- 公告摘要和“查看公告”按钮直接显示在顶部，启动等待时间位于系统列表工具栏，版本与云更新状态位于项目动态区域。
- 系统列表使用透明未选中行、独立圆角选中态、备注 Pill 和状态圆点；双击备注可以就地编辑，Enter 保存、Esc 撤销。
- 网维检测结果在固定高度卡片内原位更新，不会推动引导系统区域；状态文字和自绘反馈由零依赖的中央动画调度器统一驱动，按钮按压、加载、选择和状态更新采用 90-160ms 短缓动，并跟随 Windows 客户端动画设置。
...
界面使用 OrbiEn Desktop 的浅色令牌：主色 `#3B82F6`、工作区 `#F0F5FA`、辅助层 `#F7FAFC`、面板 `#FFFFFF`、正文 `#1E293B` 和边界 `#E2E8F0`。

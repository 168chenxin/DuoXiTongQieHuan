...
- 从旧版首次升级到 `v1.6.0` 时会完成一次品牌迁移：核心程序改名为 `SysSwitch.exe`，已有桌面和开始菜单快捷方式同步改为“系统切换大师”；后续更新继续原地替换。
- 品牌迁移会同步更新 Windows“已安装的应用”中的名称、版本、图标和卸载入口；卸载成功后自动清理迁移生成的程序与快捷方式。

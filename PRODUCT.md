# Product

## Brand

- 软件品牌全称：系统切换大师（SysSwitch Master）
- 对外展示名称：系统切换大师
- 开发部署标识：SysSwitch Master
- 核心程序与安装目录短名：SysSwitch
- GitHub 仓库：SysSwitch-Master

## Register

product

## Users

在一台电脑上维护两个 Windows 系统、需要在重启前快速选择默认启动系统的人。使用时通常正在结束当前工作，希望确认目标分区后用一次明确操作完成切换。

## Product Purpose

读取 Windows 启动菜单中的可选系统，让用户安全地设定下次默认启动项，并按需立即重启。成功的标准是用户不必再进入 msconfig，也不会误选隐藏恢复加载器。

## Brand Personality

克制、可靠、直接。

## Anti-references

采用 `ui-modern-dash` 的浅色现代后台视觉，但不要照搬网页营销 Hero、广告模块或堆叠数据卡。不要使用紫色发光效果、复杂渐变或只有颜色却没有状态含义的装饰。

## Design Principles

- 让当前默认系统和即将切换的目标一眼可辨。
- 切换默认系统和立即重启需要明确确认；启动等待时间在独立设置窗口中直接保存。
- 使用熟悉的 Windows 表格、按钮和提示交互；由 AntdUI 统一绘制状态，不混用多套现代控件库。
- 使用 `ui-modern-dash` 的靛蓝、浅靛蓝、浅灰背景和深蓝灰文字建立一致层级。
- 分发包不依赖运行时旁的资源文件。
- 每个启动项支持保存用途备注，备注按当前用户的 BCD 标识保存，不改变 Windows 引导配置。
- 启动菜单等待时间必须始终可见；在设置窗口保存后直接写入 BCD，设置为 0 秒时明确提示会跳过系统选择。
- 发布 exe 内嵌图标和全部运行代码，用户不需要下载额外 DLL、图片或 SDK。
- 选中系统拥有独立详情区，名称、分区、状态和用途备注必须在执行操作前同时可见。

## Accessibility & Inclusion

正文与背景保持高对比，状态不只依赖颜色表达。使用 Windows 自带字体并支持系统 DPI 缩放；只为悬停、按压、选择和状态更新提供短动画，并跟随 Windows 的界面动画设置。
